...
def train():
    ...
    # 网络定义
    RegressionNet = network_cfg.network.cuda()
    # 定义损失函数
    ...
    trainable_paras = filter(lambda x: x.requires_grad, RegressionNet.parameters())
    ...
    for epoch in range(network_cfg.total_epochs): 
        ...
        if (epoch+1) % network_cfg.checkpoint_save_interval == 0:
            torch.save(RegressionNet.state_dict(), network_cfg.checkpoints_dir+"/{}.pth".format(epoch+1))
    writer.close()
