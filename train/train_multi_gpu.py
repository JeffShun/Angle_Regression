...
def train():  
    ...
    # 网络定义
    RegressionNet = network_cfg.network.to(device)
    ...
    RegressionNet = torch.nn.SyncBatchNorm.convert_sync_batchnorm(RegressionNet)
    ...
    if os.path.exists(network_cfg.load_from):
        ...
        RegressionNet.load_state_dict(torch.load(network_cfg.load_from, map_location=network_cfg.device))
    else:
        # 如果不存在预训练权重，需要将第一个进程中的权重保存，然后其他进程载入，保持初始化权重一致
        if rank == 0:
            torch.save(RegressionNet.state_dict(), init_weight)
        dist.barrier()
        # 这里注意，一定要指定map_location参数，否则会导致第一块GPU占用更多资源
        RegressionNet.load_state_dict(torch.load(init_weight, map_location=device))

    # 转为DDP模型
    RegressionNet = torch.nn.parallel.DistributedDataParallel(RegressionNet, device_ids=[network_cfg.gpu])
    ...
    optimizer = optim.AdamW(params=RegressionNet.parameters(), lr=network_cfg.lr, weight_decay=network_cfg.weight_decay)
    ...
    for epoch in range(network_cfg.total_epochs): 
        ...
        for ii, (image, label) in enumerate(train_dataloader):
            image = V(image).cuda()
            label = V(label).cuda()
            regressions = RegressionNet(image)
            t_loss = train_loss_f(regressions, label)
        ...
        if (epoch+1) % network_cfg.valid_interval == 0:
            ...
            for ii, (image, label) in enumerate(valid_dataloader):
                ...
                with torch.no_grad():
                    regressions = RegressionNet(image)
                    v_loss = valid_loss_f(regressions, label)   
                    if (ii+1) % 1 == 0: 
                        save_dir = network_cfg.log_dir+"/sample/{}_{}".format(epoch+1, ii+1)
                        os.makedirs(save_dir, exist_ok=True)
                        save_result_as_image(image, regressions, label, save_dir)
        ...
        if (epoch+1) % network_cfg.checkpoint_save_interval == 0:
            torch.save(RegressionNet.module.state_dict(), network_cfg.checkpoints_dir+"/{}.pth".format(epoch+1))
    # 删除临时缓存文件
    if rank == 0:
